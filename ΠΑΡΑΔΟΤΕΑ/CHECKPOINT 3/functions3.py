...
def draw_high_score(wn,High_score,font_high_score):
    high_score_text = font_high_score.render("Highscore: "+ str(High_score),True,(255,255,255))
    wn.blit(high_score_text, (800, 20))
'''
draw menu:
'''
def draw_menu(wn,Highscore,font_high_score,MENU_picture,animation1_group,animation2_group,exit_button_group,title_group,skins_button_group,play_button_group):
    wn.blit(MENU_picture,(0,0))
    animation1_group.draw(wn)
    animation2_group.draw(wn)
    exit_button_group.draw(wn)
    title_group.draw(wn)
    title_group.update()
    draw_high_score(wn,Highscore,font_high_score)
    skins_button_group.draw(wn)
    play_button_group.draw(wn)
    pygame.display.update()
'''
Εμφανίζεται στην οθόνη ένα μύνημα το οποίο ζητάει από το χρήστη να επιλέξει skin για το διαστημόπλοιο. 
'''
def draw_skin_TEXT(wn,font_skins):
...
'''
def draw_skin_menu(wn,font_skins,skin_1_group,skin_2_group,skin_3_group,skin_4_group,skin_5_group,menu_button_group):
    wn.fill((0,0,0))
