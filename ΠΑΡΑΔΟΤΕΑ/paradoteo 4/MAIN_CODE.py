    ...
    while CO_OP:
        ...
        if not stop_game:
            ...
            if len(red_pipe_group) > 1 and len(blue_pipe_group) > 1:
                ...
                if pygame.sprite.collide_mask(Player_2, red_pipe_group.sprites()[0]) or pygame.sprite.collide_mask(Player_2, red_pipe_group.sprites()[1]):
                    if not(pygame.sprite.collide_mask(Player_1, blue_pipe_group.sprites()[0]) or pygame.sprite.collide_mask(Player_1, blue_pipe_group.sprites()[1])):
                        game_over = True
                        player_1_winner = True
                        player_2_winner = False
                        DRAW = False
                # Αν ο player 1 ακουμπήσει εμπόδιο αλλά ο player 2 δεν ακουμπάει εμπόδιο τότε νικάει ο player 2
                if pygame.sprite.collide_mask(Player_1, blue_pipe_group.sprites()[0]) or pygame.sprite.collide_mask(Player_1, blue_pipe_group.sprites()[1]):
                    if not(pygame.sprite.collide_mask(Player_2, red_pipe_group.sprites()[0]) or pygame.sprite.collide_mask(Player_2, red_pipe_group.sprites()[1])):
                        game_over = True
